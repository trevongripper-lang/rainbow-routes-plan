...
const RPCS: Array<{ name: string; args: Record<string, unknown> }> = [
  {
    name: "is_trip_member",
    args: {
      _dest: "00000000-0000-0000-0000-000000000000",
...
export const runRlsSmokeTests = createServerFn({ method: "POST" })
  ...
  .handler(
    async ({ context }): Promise<{ checks: SmokeCheck[]; ran_at: string; user_id: string }> => {
      ...
      const rpcChecks = await Promise.all(
        RPCS.map<Promise<SmokeCheck>>(async ({ name, args }) => {
          ...
          const permissionDenied =
            code === "42501" || /permission denied/i.test(error?.message ?? "");
          return {
            kind: "rpc",
            name,
            ok: !permissionDenied,
