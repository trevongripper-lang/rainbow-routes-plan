...
async function fetchRatingData(id: string, me: string | undefined) {
  ...
  const a = (
    agg as {
      data?: Array<{ avg_rating: number | null; rating_count: number; feedbacks: string[] }>;
    } | null
  )?.data?.[0] ?? { avg_rating: null, rating_count: 0, feedbacks: [] };
  return {
    agg: a,
    mine: (mine as { data?: { rating: number; feedback: string | null } } | null)?.data ?? null,
  };
}
