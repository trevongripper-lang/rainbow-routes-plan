...
CREATE INDEX IF NOT EXISTS destinations_latlng_idx
  ON public.destinations (latitude, longitude)
...
BEGIN
  SELECT latitude, longitude, region, country, start_date, end_date
    INTO d_lat, d_lng, d_region, d_country, d_start, d_end
  FROM public.destinations
  WHERE destinations.id = _dest;
  ...
  WHERE
    -- distance filter: prefer coords; fall back to region/country match
    (
...
GRANT EXECUTE ON FUNCTION public.match_trip_events(uuid, numeric, integer, boolean) TO authenticated, anon;
